    ...
    const [payments, demurrage, stale, missingDocs, arriving, slippage, anomaly] = await Promise.all([
      db.query(`
        ...
        WHERE o.payment_due_date IS NOT NULL AND o.status NOT IN ('Delivered')
          ...
          AND o.payment_due_date <= CURRENT_DATE + INTERVAL '14 days'
      ...
      db.query(`
        SELECT o.id, o.po_number, s.name AS supplier, o.eta::date, o.free_days,
               ...
               GREATEST(0, DATEDIFF(COALESCE(o.container_returned_date, CURRENT_DATE), o.eta) - COALESCE(o.free_days, 7)) AS over_days
        FROM import_orders o
        JOIN suppliers s ON o.supplier_id = s.id
        WHERE o.eta IS NOT NULL
          AND o.status IN ('Arrived','Customs Clearance','Cleared')
          AND o.container_returned_date IS NULL
          AND DATEDIFF(CURRENT_DATE, o.eta) > COALESCE(o.free_days, 7) - 2
        ORDER BY over_days DESC
      `),
      db.query(`
        SELECT o.id, o.po_number, s.name AS supplier, o.status,
               DATEDIFF(CURRENT_DATE, o.updated_at) AS days_idle
        FROM import_orders o
        JOIN suppliers s ON o.supplier_id = s.id
        WHERE o.status NOT IN ('Delivered')
          AND o.updated_at < CURRENT_DATE - INTERVAL '14 days'
      ...
      db.query(`
        SELECT o.id, o.po_number, s.name AS supplier, o.eta::date, o.container_type,
               DATEDIFF(o.eta, CURRENT_DATE) AS days_to_eta
        FROM import_orders o
        JOIN suppliers s ON o.supplier_id = s.id
        WHERE o.eta IS NOT NULL AND o.status IN ('Shipped','In Transit')
          AND o.eta BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'
        ORDER BY o.eta
      `),
      db.query(`
        SELECT o.id, o.po_number, s.name AS supplier, o.status,
               CASE WHEN o.status IN ('Confirmed','Loaded') THEN 'etd' ELSE 'eta' END AS kind,
               CASE WHEN o.status IN ('Confirmed','Loaded') THEN DATEDIFF(CURRENT_DATE, o.etd)
                    ELSE DATEDIFF(CURRENT_DATE, o.eta) END AS days_late
  ...
  try {
    const [ship, arrive, pay, exc] = await Promise.all([
      db.query(`SELECT COUNT(*)::int n, COALESCE(SUM(total_value),0)::float v FROM import_orders
                WHERE status IN ('Confirmed','Loaded') AND etd::date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'`),
      db.query(`SELECT COUNT(*)::int n FROM import_orders
                WHERE status IN ('Shipped','In Transit') AND eta::date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'`),
      db.query(`SELECT COUNT(*)::int n,
                ...
                FROM import_orders o
                WHERE payment_due_date IS NOT NULL AND payment_due_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'
                  AND total_value - COALESCE((SELECT SUM(amount) FROM payments p WHERE p.order_id=o.id),0) > 0.5`),
      db.query(`SELECT
                  COUNT(*) FILTER (WHERE status NOT IN ('Delivered','Paid','Draft') AND updated_at < CURRENT_DATE - INTERVAL '14 days')::int AS stale,
