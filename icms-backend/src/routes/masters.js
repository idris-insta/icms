...
const SUPPLIER_HEADERS = 'code,name,port,city,country,base_currency,contact_email,contact_phone,payment_terms_days';
...
const SUPPLIER_EXAMPLE = 'SUP-001,Acme Packaging Ltd,QINGDAO,WEIFANG,China,USD,contact@acme.cn,+86-21-12345678,30';
...
//  Backing Material, Brand, Liner, Item Name, Density, Default Unit of Measure).
function mapSkuRow(raw) {
  const m = {};
  for (const k of Object.keys(raw)) m[hkey(k)] = (raw[k] ?? '').trim();
  const num = (v) => { const n = parseFloat(v); return Number.isFinite(n) ? n : null; };
  const int = (v) => { const n = parseInt(v); return Number.isFinite(n) ? n : null; };

  const sku_code = m.skucode || m.itemcode || m.code || '';
  const width  = num(m.widthmm || m.width);
  const length = num(m.lengthmtr || m.lengthmm || m.length);
  // Build a readable size string when explicit size absent
  let size = m.size || '';
  if (!size && (width || length)) size = `${width || '?'}MM x ${length || '?'}MTR`;
  // Description: prefer explicit description, else the item name
  const description = m.description || m.itemname || m.name || sku_code;

  return {
    sku_code,
    description,
    hsn_code:         m.hsncode || m.hsnsac || m.hsn || null,
    category:         m.category || m.itemgroup || m.group || null,
    thickness:        m.thickness || null,
    size:             size || null,
    color:            m.color || m.colour || null,
    liner_color:      m.linercolor || m.liner || null,
    roll_weight:      num(m.rollweight),
    item_code:        m.itemcode || sku_code || null,
    shipping_marks:   m.shippingmarks || m.itemname || null,
    weight_per_unit:  num(m.weightperunit),
    cbm_per_unit:     num(m.cbmperunit),
    brand:            m.brand || null,
    uom:              m.uom || m.defaultunitofmeasure || m.unit || null,
    qty_per_pkg:      int(m.qtypkg || m.qtyperpkg || m.qtypkgs),
    adhesive_type:    m.adhesivetype || m.adhesive || null,
    backing_material: m.backingmaterial || m.backing || null,
    width_mm:         width,
    length_mtr:       length,
    ...
    try {
      const result = await db.query(`
        ...
        ON CONFLICT (sku_code) DO UPDATE SET
          ...
          uom            = EXCLUDED.uom,
          qty_per_pkg    = EXCLUDED.qty_per_pkg,
          adhesive_type  = EXCLUDED.adhesive_type,
          backing_material = EXCLUDED.backing_material,
...
router.put('/skus/:id', protect, authorize('owner', 'manager'), async (req, res) => {
  const b = req.body;
  // text '' stays as set value; numeric '' -> null so it doesn't break the cast
  const txt = (v) => (v === undefined ? null : v);
  const nnum = (v) => { if (v === '' || v === undefined || v === null) return null; const n = parseFloat(v); return Number.isFinite(n) ? n : null; };
  const nint = (v) => { if (v === '' || v === undefined || v === null) return null; const n = parseInt(v); return Number.isFinite(n) ? n : null; };
  try {
    const { rows } = await db.query(`
      UPDATE skus SET
        ...
        uom              = COALESCE($14, uom),
        qty_per_pkg      = COALESCE($15, qty_per_pkg),
        adhesive_type    = COALESCE($16, adhesive_type),
        backing_material = COALESCE($17, backing_material),
        width_mm         = COALESCE($18, width_mm),
        length_mtr       = COALESCE($19, length_mtr),
        density          = COALESCE($20, density),
        updated_at      = NOW()
      WHERE id = $21 RETURNING *
    `, [txt(b.description), txt(b.hsn_code), nnum(b.weight_per_unit), nnum(b.cbm_per_unit), txt(b.category),
        txt(b.thickness), txt(b.size), txt(b.color), txt(b.liner_color), nnum(b.roll_weight), txt(b.item_code), txt(b.shipping_marks),
        txt(b.brand), txt(b.uom), nint(b.qty_per_pkg), txt(b.adhesive_type), txt(b.backing_material),
        nnum(b.width_mm), nnum(b.length_mtr), txt(b.density),
        req.params.id]);
